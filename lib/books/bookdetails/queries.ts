...
export const getEditionTitleCached = (editionId: string) =>
  unstable_cache(
    async () => {
      return getEditionTitle(editionId);
    },
    ["edition-title", editionId],
    {
      revalidate: 60 * 60 * 24 * 30, // 30 days
    }
  )();

export async function getEditionTitle(editionId: string) {
...
export async function getEditionCover(editionId: string) {
  return await prisma.edition.findFirstOrThrow({
    where: {
      id: editionId,
    },
    select: {
...
export async function getEditionDescription(editionId: string) {
  return await prisma.edition.findFirstOrThrow({
    where: {
      id: editionId,
    },
...
export async function getBookSpecs(editionId: string) {
  const res = await prisma.edition.findUniqueOrThrow({
    where: {
      id: editionId,
    },
    select: {
...
export async function getBookTitleSectionData(editionId: string) {
  const res = await prisma.edition.findUniqueOrThrow({
    where: {
      id: editionId,
    },
  ...
  return {
    ...
    authors: res.book.authors.map((a) => ({
      name: a.person.name,
      slug: a.person.slug,
      order: a.order ?? null,
    })),
    genres: res.book.genres.flatMap((g) => {
